...
impl Node<'_> {
    pub fn new(cluster: &str) -> Self {
        let session = SyncResolve::res(zenoh::open(config::default()))
            .unwrap()
            .into_arc();
    ...
    pub async fn start_async(&mut self) {
        ...
        let subscription = AsyncResolve::res(
            self.session
                .declare_subscriber(format!("{}/node/{}/*", node_data_guard.cluster_name, node_data_guard.zid))
                .reliable()
                .callback(move |sample| {
                    ...
                    match topic {
                        "new_reply" => {
                            handle_owner_request(payload, node_data_guard_clone, session_clone.clone());
                        }
                        "owner_request" => {
                            handle_owner_request(payload, node_data_guard_clone, session_clone.clone());
                        }
                        "owner_response" => {
                            handle_owner_response(payload, node_data_guard_clone, session_clone.clone());
                        }
                        ...
                        "new_voronoi" => {
                            handle_new_voronoi_request(payload, node_data_guard_clone, session_clone.clone());
                        }
                        "leave_voronoi" => {
                            handle_leave_voronoi_request(payload, node_data_guard_clone, session_clone.clone());
                        }
                        "leave_reply" => {
                            handle_leave_response(payload, node_data_guard_clone, session_clone.clone());
                        }
        ...
        SyncResolve::res(self.session.put(
